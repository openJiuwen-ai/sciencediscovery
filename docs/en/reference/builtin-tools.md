...
| `materialize_skill_resource` | a selected skill has resources | `skillId` + snapshot `path` + optional workspace-relative `dest`; writes exact frozen bytes to the writable Session workspace and returns metadata only. It never executes or installs the file, and selecting the Skill alone copies nothing |
