...
| `gateway-agent.ts` | 经 Gateway 侧车执行 Agent；工具回调 token 注册表 |
...
| `mcp/` | MCP 治理：`broker.ts`（调用治理）、`source-catalog.ts`、`artifact-manager.ts`（下载/抽取任务）、`rate-limiter.ts`、`result-cache.ts` |
...
| `POST /internal/tool-exec` | gateway 代理工具回调（仅回环；per-run Bearer token） |
...
| `POST /api/sessions/:id/runs/:runId/cancel` | 取消运行（abort 贯通到 gateway 与 runner） |
...
| `/api/settings`、`/api/timeout-settings`、`/api/runtime-status` | 全局设置、超时、运行状态页 |
...
2. **执行**：创建 `RequestExecutionContext`（executionId、权限运行时、abort signal），经 `GatewayAgent` 调 Gateway。
3. **记录**：写 Prompt Manifest 与执行、Artifact 溯源记录；显式触发的 Reviewer Specialist 通过独立 checkpoint 审核 Artifact，不阻塞主 Agent。
4. **收尾**：发 `run.completed`/`run.failed`/`run.cancelled`，清理挂起权限请求与回调 token。

子 Agent（`task` 工具）在私有工作区中运行受限工具集，交接文件有大小上限，结束时按 Brief 的 `outputJsonSchema` 校验结构化输出（契约见 [agent-backend.md](agent-backend.md#61-subagent-brief-v1-契约)）。
...
- 每次运行生成随机 `callbackToken` 注册进 `toolCallbackRegistry`；gateway 的代理工具带 `Authorization: Bearer <token>` 回调 `/internal/tool-exec`，未知 token 返回 401。运行结束即注销。
...
- **论文视觉分析**（`papers.ts`）直接 `fetch` OpenAI 兼容端点，不经 gateway；细节见 [PDF worker 参考](../reference/paper-worker.md)。
