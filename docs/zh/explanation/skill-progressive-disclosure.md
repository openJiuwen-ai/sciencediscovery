...
prepareSkillSandbox 把完整技能包写入本次执行的 snapshot 根目录
        ...
        ▼
沙箱启动时这些包已经挂好
        ├─ /skills            只读，$SCIENCEDISCOVERY_SKILLS_DIR
        │     └─ <skillId>/SKILL.md、scripts/、references/、assets/…
        └─ /skill-extensions  可写，$SCIENCEDISCOVERY_SKILL_EXTENSIONS_DIR
        │
        ▼
system prompt 逐个列出 <package_path> 与 <package_hash>
        │
        ▼
用 read_file 读取 /skills/<skillId>/SKILL.md（read_skill 作为兼容通道保留）
...
| 路径 | 权限 | 用途 |
|------|------|------|
| `/skills/<skillId>` | 只读 | 一个已选技能的完整冻结包，含 `SKILL.md`、`scripts/`、`references/` 和其它资源 |
| `/skills/.sciencediscovery-snapshot.json` | 只读 | 清单文件，记录每个已放入技能的 id、revision、version 和包 hash |
| `/skill-extensions` | 可写 | 为后续自演进预留的扩展目录；默认为空，且不属于冻结树 |

在 `run_shell`、`run_python`、`run_r` 内部，`$SCIENCEDISCOVERY_SKILLS_DIR` 和 `$SCIENCEDISCOVERY_SKILL_EXTENSIONS_DIR` 会解析到上述位置，因此技能说明可以引用环境变量，而不必写死挂载路径。

## 工具职责

| 工具 | 执行位置 | 职责 |
|------|----------|------|
| `read_file` | Node 工作区工具 | 像读工作区文件一样，分页读取 `/skills` 下任意已放入的包文件 |
...
- 只有本次运行已选的技能会被放入；未选中的技能不会出现在 `/skills` 下。
- 默认包目录只读：沙箱内对 `/skills` 的写入和删除都会失败，宿主侧文件权限为 `0444`。
