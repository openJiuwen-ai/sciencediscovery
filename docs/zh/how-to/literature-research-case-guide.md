...
本指南是 ScienceDiscovery 的使用手册，指南以一项跨数据库文献调研任务为线索，演示从启动服务、配置系统，到下发任务、审批与查看结果的全流程。
...
最终智能体输出一份带有可点击引用标签的报告。每个标签对应具体引用内容、来源文献及执行链路，用户点击即可核验。这彻底消除了大模型的“幻觉”问题，保证了报告的真实性，将数天的文献调研压缩至分钟级，同时获得透明的审查机制，快速验证每一项数据的出处。
