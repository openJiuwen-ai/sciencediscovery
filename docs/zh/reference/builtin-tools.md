...
| `materialize_skill_resource` | 选中的技能中至少一个带资源 | `skillId` + snapshot `path` + 可选工作区相对 `dest`；把冻结原始字节写入当前 Session 可写工作区，只返回 metadata。工具不执行、不安装文件；仅选择技能时不会复制任何资源 |
...
技能加载流程见 [skill-progressive-disclosure.md](../explanation/skill-progressive-disclosure.md)：模型从 Prompt 中的目录 metadata 选择精确 `skillId`，再按资源用途用 `read_skill`、`read_skill_resource` 或 `materialize_skill_resource` 访问本次运行的冻结快照。
