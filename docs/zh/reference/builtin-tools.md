...
技能加载流程见 [skill-progressive-disclosure.md](../explanation/skill-progressive-disclosure.md)。本次运行已选技能的**完整冻结包**在沙箱启动前就已放入只读的 `/skills/<skillId>`（沙箱内为 `$SCIENCEDISCOVERY_SKILLS_DIR`），Prompt 逐个给出包路径和包 hash。因此 `read_file` 可以直接分页读取 `/skills/...` 下的包文件，`run_shell` 的 `scriptPath` 也可以直接指向包内脚本并用 `arguments` 传显式 argv，无需先复制到工作区。`/skill-extensions`（`$SCIENCEDISCOVERY_SKILL_EXTENSIONS_DIR`）是为后续自演进预留的可写目录，默认为空。放入技能包**不等于**自动执行或安装其中的 `scripts/`；执行必须由 Agent 显式发起。`read_skill` / `read_skill_resource` 作为兼容通道保留。
