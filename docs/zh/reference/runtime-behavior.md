...
启用连接器或具备运行时**不等于**执行授权。首次匹配的代码或连接器动作会暂停并弹出权限卡片（除非已有未撤销授权）。持久授权列于 **系统配置 → Permissions**；撤销后下一次匹配动作会再次询问。目录类工具仍限制在工作区内，授权**不会**关闭 bubblewrap 文件系统隔离或 `networkPolicy=none`。
