...
export function shortModelProfileId(id: string): string {
  return id.length <= 12 ? id : `${id.slice(0, 6)}…${id.slice(-4)}`;
}

export function duplicateModelProfileId(
  profile: ModelProfile,
  profiles: ModelProfile[],
): string | undefined {
  const identity = modelIdentity(profile);
  return profiles.filter((candidate) => modelIdentity(candidate) === identity).length > 1
    ? shortModelProfileId(profile.id)
    : undefined;
}
