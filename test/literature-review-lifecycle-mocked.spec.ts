...
/** Real Swarm/Adapter/API/browser; only the external LLM is scripted. */
test("LR-16 upstream 503 cancellation leaves the same session and shared MCP usable", { tag: "@mocked" }, async ({ page, journey }, info) => {
  info.skip(process.env.E2E_SWARM_TASK !== "1", "BLOCKED: requires Swarm task stack");
 ...
 */
test(`${id} ${names[id]}`, { tag: "@mocked" }, async ({ page, journey }, info) => {
  info.skip(process.env.E2E_SWARM_TASK !== "1" || process.env.E2E_SWARM_EXCLUSIVE !== "1",
    "BLOCKED: needs an exclusive Swarm test stack; this case temporarily changes global quota settings");
  test.setTimeout(180_000);
