...
| `test/api/sciencediscovery-npu.test.mjs` | 1 | `npu:required`, `model:real`, `status:external` | Creates a fresh Project/Session, uploads user PDBs, and asks a live JiuwenSwarm Agent to complete the whole `antibody-design` Skill without preselecting an environment or invoking scripts from the driver. It verifies Skill loading, remote input sync, first-use preparation, validation, managed NPU execution (retry only after a confirmed failed receipt), model stages, output transfer, Artifacts, and the Agent's answer. Requires explicit live-model/NPU opt-in; PR CI collects but deselects it. |
