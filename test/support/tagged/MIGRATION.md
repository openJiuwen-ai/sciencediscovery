...
| `test/api/sciencediscovery-npu.test.mjs` | 1 | `npu:required`, `model:real`, `status:external` | Exercises the JiuwenSwarm Agent → `antibody-design` Skill → managed `run_shell` → remote NPU Runner → committed receipt → output sync. Requires an explicitly prepared Session and `SCIENCEDISCOVERY_NPU_E2E_*` opt-in settings; PR CI collects but deselects it. |
