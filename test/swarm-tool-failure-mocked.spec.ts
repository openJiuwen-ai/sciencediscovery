 ...
 */
test(`${fault === "disconnect" ? "LR-07 " : ""}Swarm ${fault} child failure is visible after recovery`, { tag: "@mocked" }, async ({ page, journey }, info) => {
  info.skip(process.env.E2E_SWARM_TASK !== "1", "Requires isolated Swarm stack with platform task delegation");
  info.skip(transportFault && process.env.E2E_MCP_FAULT_PROXY !== "1", "Requires isolated loopback MCP fault proxy");
