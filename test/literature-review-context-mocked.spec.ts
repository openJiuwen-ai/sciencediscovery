...
test(`${id} ${id === "LR-08" ? "actual Swarm compression retains recoverable evidence" : "single-line output supports bounded search, character ranges and duplicate-read advice"}`,
  { tag: "@mocked" }, async ({ page, journey }, info) => {
  info.skip(process.env.E2E_SWARM_TASK !== "1", "BLOCKED: requires Swarm with platform tools");
  info.skip(id === "LR-08" && process.env.E2E_SWARM_COMPACTION !== "1", "BLOCKED: requires isolated Swarm context-engine configuration; do not alter a live research instance");
