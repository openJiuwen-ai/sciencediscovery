...
if [[ "$cache_only" == 1 ]]; then
  echo "Fetching the pinned Ubuntu cloud image (local cache, then public cache; source fallback disabled)"
else
  echo "Fetching the pinned Ubuntu cloud image (local cache, public cache, then TUNA)"
fi
