...
- Do not materialize or manually copy files from `scripts/` / `resources/` with `materialize_skill_resource` or `read_skill_resource`; those package files are for the Broker adapter and operator deployment path, not for Agent-authored execution.
