...
3. **起点。** 工作区里有现成的就用；没有就你写一份**朴素的**——固定步长、简单规则、
   最普通的模型。你知道最优解不代表该把最优解写成起点，那样搜索就没得可搜了。
...
**思考默认关掉**，除非用户明确要开：实测某端点上开着思考一次整程序改写可能超过 15 分钟不返回，
撞上代理上限被掐断，那次扩展既没有候选也没有 token。
...
树没有梯度可爬——一次真跑里六个正经的自适应方法全部因此得 0。
...
候选运行时保底有 numpy / pandas / scipy / sklearn 和标准库。这个任务还需要别的
（表格回归想用 lightgbm / xgboost、时间序列想用 statsmodels）就写进 `packages`，探针之前会装好。
**只写包名**（可带 `==版本`），路径、URL、索引地址、pip 选项一律不行。用不上的别写。

注意你自己的 `run_python` 环境和候选的沙箱环境不是一个——验证时能 import 不代表候选也能，
`packages` 该写还是要写。
