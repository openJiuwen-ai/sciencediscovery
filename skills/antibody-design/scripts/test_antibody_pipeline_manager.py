...
class SandboxConfigTests(unittest.TestCase):
    ...
    def test_rfdiffusion_final_step_must_fit_timestep_count(self) -> None:
        base = {
            "design_loops": "[H1:8]",
            "hotspots": "[A45]",
            "npus": "0",
        ...
        self.assertEqual(
            manager.validate_format({**base, "diffuser_t": 15, "final_step": 1}),
            [],
        )
        self.assertIn(
            "final_step must be less than or equal to diffuser_t",
            manager.validate_format({**base, "diffuser_t": 15, "final_step": 160}),
        )
