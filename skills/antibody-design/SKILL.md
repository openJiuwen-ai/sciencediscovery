...
launched. Keep `diffuser_t >= 15` and `1 <= final_step <= diffuser_t`; for a
short smoke run, use `diffuser_t=15` with `final_step=1`. Reusing a run name
with `force=true` deletes that run's existing stage outputs, so require explicit
overwrite intent.
