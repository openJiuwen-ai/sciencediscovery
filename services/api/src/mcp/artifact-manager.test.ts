...
test("artifact manager derives an immutable plan from MCP CAS data and downloads after approval", async (context) => {
...
test("artifact manager resumes concurrent downloads without corrupting shared job state", async (context) => {
...
test("artifact manager waits for a pending permission and returns denial as a terminal result", async (context) => {
  ...
  const manager = new ArtifactManager(store, registry, broker);
  const creation = await manager.prepare(session.id, {
...
test("artifact manager rejects a candidate whose host is outside the source manifest", async (context) => {
