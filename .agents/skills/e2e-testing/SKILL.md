...
| `SCIENCE_AGENT_RUNNER_PORT` / `SCIENCE_AGENT_GATEWAY_PORT` | `4311` / `4312` | where runner/gateway listen |
| `SCIENCE_AGENT_RUNNER_URL` / `SCIENCE_AGENT_GATEWAY_URL` | `http://127.0.0.1:<port>` | **where the API dials** — the API does not read the `*_PORT` variables, so on non-default ports always set the full URLs too |
| `SCIENCE_AGENT_DATA_DIR` | `data` | data root; also holds `envs/` (service venvs) |
...
- **Never share `data/envs` between worktrees** (no symlinks). The service
